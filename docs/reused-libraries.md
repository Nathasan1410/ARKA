...
Status: source fork present; local install, strict-smoke build, direct source CLI help/version/gateway-help, local dev gateway connectivity, ARKA skill loading, MiniMax model discovery, and read-only plugin static smoke verified; full build, model-backed ARKA turn, gateway discovery/load of the plugin, and packages/agent gateway calls not verified yet
```
